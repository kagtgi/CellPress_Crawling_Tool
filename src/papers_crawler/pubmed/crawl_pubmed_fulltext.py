...
async def process_pmc_articles(pmc_ids, pdf_output=None, json_output=None):
    ...
    async with async_playwright() as p:
        ...
        for idx, pmc_id in enumerate(pmc_ids, 1):
            print(f"[{idx}/{len(pmc_ids)}] Processing {pmc_id}...")
...
async def main():
    parser = argparse.ArgumentParser(description="Crawl PubMed papers fulltext (JSON and PDF)")
    ...
    if pmc_ids:
        ...
        print("No open-access articles or PMC IDs found to process.")
